...
import { Loader2, ShieldAlert, UserX, LogIn, CheckCircle2, ChevronRight, AlertTriangle, Shield, BookOpen, ChevronLeft } from "lucide-react";
...
export default function ApplyPage({ guildId, panelId }: { guildId: string; panelId: string }) {
  ...
  // Screen flow: "intro" → "form"
  const [screen, setScreen] = useState<"intro"|"form">("intro");
  // Current question index for step-by-step (we show all at once but track progress)
    ...
    <div className="min-h-screen bg-[#0d0f14] flex flex-col items-center justify-center p-6 text-white">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-6">
          ...
          {panel?.description && <p className="text-white/50 text-sm max-w-sm">{panel.description}</p>}
        </div>
        ...
        {panel?.required_role_id && (
          <div className="bg-purple-500/10 border border-purple-500/20 rounded-xl p-3 mb-4 flex items-center gap-2.5">
            ...
            <p className="text-xs text-purple-300">This application requires a specific server role. Make sure you have it before applying.</p>
          </div>
        )}
        ...
        {/* Apply button */}
        <Button
          className="gap-2 font-bold px-8 py-3 text-base rounded-xl shadow-lg w-full"
        ...
        <p className="text-center text-white/20 text-xs mt-5">Powered by <a href="https://zenithbot.up.railway.app/" target="_blank" rel="noreferrer" className="hover:text-white/30 transition-colors">Zenith</a></p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#0d0f14] flex flex-col items-center py-10 px-4 text-white">
      <div className="w-full max-w-lg">
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-white/40 hover:text-white/60 text-xs mb-5 transition-colors">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {effectiveRules && (
            <div className="bg-white/5 border border-white/10 rounded-xl p-3 space-y-1.5">
              <div className="flex items-center gap-1.5 text-[10px] text-white/40 uppercase tracking-wider font-semibold">
                <BookOpen size={9} /> Rules
              </div>
              <p className="text-xs text-white/50 whitespace-pre-wrap leading-relaxed line-clamp-3">{effectiveRules}</p>
            </div>
          )}
          ...
          {effectiveRules && !submitting && (
            <p className="text-center text-white/25 text-[11px] mt-1 leading-relaxed">
              By clicking <strong className="text-white/40">{buttonLabel}</strong>, you agree to the rules above.
            </p>
          )}
        </form>
