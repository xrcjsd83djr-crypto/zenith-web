...
import {
  Loader2, ShieldAlert, UserX, LogIn, CheckCircle2, ChevronRight,
  AlertTriangle, Shield, BookOpen, ChevronLeft, X, Eye, EyeOff,
} from "lucide-react";
...
interface Question {
  id: string; text: string; type: "short" | "long" | "choice"; required: boolean; choices?: string[];
}
interface Panel {
...
export default function ApplyPage({ guildId, panelId }: { guildId: string; panelId: string }) {
  ...
  const effectiveRules = panel?.rules?.trim() || panel?.guild_rules?.trim() || "";
  const questions = panel?.questions || [];
  const answeredCount = questions.filter(q => answers[q.id]?.trim()).length;
  const progressPercent = questions.length > 0 ? Math.round(answeredCount / questions.length * 100) : 0;
  const buttonLabel = panel?.button_label || "Apply Now";
  ...
  if (!user) return (
    <div className="min-h-screen bg-[#0d0f14] flex flex-col items-center justify-center p-6 text-center text-white">
      ...
      <p className="text-white/40 text-sm uppercase tracking-widest font-semibold mb-1">{panel?.guildName}</p>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-5 mb-6 flex items-center gap-3 max-w-xs w-full">
        ...
        <p className="text-sm text-white/60 text-left">Sign in with Discord to begin your application</p>
      </div>
      <Button className="gap-2 font-bold px-8 py-3 text-base rounded-xl shadow-lg"
  ...
  if (screen === "intro") return (
    <div className="min-h-screen bg-[#0d0f14] flex flex-col items-center justify-center p-6 text-white">
      <div className="w-full max-w-sm">
        <div className="text-center mb-6">
          {panel?.guildIcon
            ? <img src={panel.guildIcon} alt="" className="w-20 h-20 rounded-2xl mx-auto mb-3 object-cover shadow-xl" />
            : <div className="w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-3 text-3xl font-black shadow-xl"
                style={{ background: "linear-gradient(135deg,#d4af37,#f0c040)", color: "#000" }}>Z</div>}
          <p className="text-white/40 text-xs uppercase tracking-widest font-semibold mb-1">{panel?.guildName}</p>
          <h1 className="text-2xl font-extrabold mb-1">{panel?.title}</h1>
          {panel?.description && <p className="text-white/50 text-sm">{panel.description}</p>}
        </div>
        ...
        <div className="space-y-2 mb-4">
          <p className="text-[11px] text-white/30 uppercase tracking-wider font-semibold text-center mb-2">What to expect</p>
          {[
            ...
            <div key={i} className="flex items-center gap-2 text-sm text-white/50">
              <span className="w-5 h-5 rounded-full bg-white/10 flex items-center justify-center text-[10px] font-bold flex-shrink-0">{i + 1}</span>
              {step}
            </div>
          ))}
        </div>
        ...
        {effectiveRules && (
          <div className="bg-white/5 border border-white/10 rounded-xl mb-5 overflow-hidden">
            <div className="px-4 py-3 flex items-center gap-2">
              <BookOpen size={13} className="text-white/50 flex-shrink-0" />
              <p className="text-xs text-white/50 font-semibold uppercase tracking-wider flex-1">Rules & Requirements</p>
            </div>
            <div className="px-4 pb-4">
              <p className="text-xs text-white/60 whitespace-pre-wrap leading-relaxed">{effectiveRules}</p>
            </div>
          </div>
        )}
        ...
        {effectiveRules && (
          <p className="text-center text-white/25 text-[11px] mt-3 leading-relaxed">
            By clicking <strong className="text-white/40">{buttonLabel}</strong>, you agree to the rules above.
          </p>
        )}
    ...
    <div className="min-h-screen bg-[#0d0f14] flex flex-col items-center py-10 px-4 text-white">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-5">
          {panel?.guildIcon
            ? <img src={panel.guildIcon} alt="" className="w-12 h-12 rounded-xl mx-auto mb-3 object-cover shadow-lg" />
            : <div className="w-12 h-12 rounded-xl mx-auto mb-3 flex items-center justify-center text-xl font-black shadow-lg"
                style={{ background: "linear-gradient(135deg,#d4af37,#f0c040)", color: "#000" }}>Z</div>}
          ...
          <div className="mb-5 space-y-1.5">
            ...
            <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
              <div className="h-full rounded-full transition-all duration-500"
                style={{ width: `${progressPercent}%`, background: "linear-gradient(90deg,#d4af37,#f0c040)" }} />
            </div>
          </div>
        )}

        {/* Back + Rules toggle row */}
        <div className="flex items-center justify-between mb-5">
          ...
          {/* Collapsible Rules button */}
          {effectiveRules && (
            <button onClick={() => setRulesOpen(o => !o)}
              className="flex items-center gap-1.5 text-xs text-white/40 hover:text-white/60 transition-colors border border-white/10 rounded-lg px-2.5 py-1.5 hover:border-white/20">
              <BookOpen size={11} />
              {rulesOpen ? "Hide Rules" : "View Rules"}
              {rulesOpen ? <EyeOff size={10} /> : <Eye size={10} />}
            </button>
          )}
        </div>

        {/* Collapsible rules panel */}
        {effectiveRules && rulesOpen && (
          <div className="mb-5 bg-white/5 border border-white/10 rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-white/5">
              <span className="text-[10px] text-white/40 uppercase tracking-wider font-semibold flex items-center gap-1.5">
                <BookOpen size={9} /> Rules & Requirements
              </span>
              <button onClick={() => setRulesOpen(false)} className="text-white/30 hover:text-white/60">
                <X size={13} />
              </button>
            </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div key={q.id} className="space-y-2">
              ...
              {q.type === "short" && (
                <Input value={answers[q.id] || ""} onChange={e => setAnswers(a => ({ ...a, [q.id]: e.target.value }))}
                  ...
                  className="bg-white/5 border-white/10 text-white placeholder:text-white/25 focus:border-[#d4af37]/50 h-10" />
              )}
              {q.type === "long" && (
          ...
          <Button type="submit" disabled={submitting} className="w-full h-11 font-bold text-base rounded-xl mt-2 gap-2"
            style={{ background: "linear-gradient(135deg,#d4af37,#f0c040)", color: "#000" }}>
            {submitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Submitting…</> : buttonLabel}
          </Button>
        </form>
