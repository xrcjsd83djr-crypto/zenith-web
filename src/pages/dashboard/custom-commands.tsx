...
function CommandCard({ cmd, prefix, onEdit, onDelete }: { cmd: Command; prefix: string; onEdit: () => void; onDelete: () => void }) {
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        <button onClick={() => setOpen(o=>!o)} className="flex-1 flex items-center gap-3 text-left hover:text-foreground">
          ...
          <div className="flex-1 min-w-0">
            <span className="font-mono font-semibold text-sm">{pfx}{cmd.name}</span>
      ...
      {open && (
        <div className="px-4 pb-4 border-t bg-muted/20">
          <div className="grid grid-cols-2 gap-3 mt-3">
            <div><p className="text-xs text-muted-foreground font-medium">Command</p><p className="text-sm font-mono">{pfx}{cmd.name}</p></div>
...
export default function CustomCommandsPage({ guildId }: { guildId: string }) {
  ...
  const [prefix, setPrefix] = useState('z!');
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      if (cfgRes.ok) { const cfg = await cfgRes.json(); if (cfg.prefix?.trim()) setPrefix(cfg.prefix.trim()); }
    } catch {}
    setLoading(false);
  }, [guildId]);
  ...
  const handleDelete = async (id:string, name:string) => {
    if (!confirm(`Delete ${prefix}${name}?`)) return;
    ...
    <div className="space-y-5 max-w-4xl">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted-foreground mt-0.5 text-sm">{commands.length}{!isPremium?'/'+FREE_LIMIT:''} commands — use <span className="font-mono">{prefix}commandname</span> in Discord</p>
        </div>
      ...
      {loading ? <div className="flex justify-center py-16"><div className="w-7 h-7 rounded-full border-2 border-t-transparent animate-spin" style={{borderColor:'#d4af37',borderTopColor:'transparent'}}/></div>
        ...
        : <div>{commands.map(c=><CommandCard key={c.id} cmd={c} prefix={prefix} onEdit={()=>openEdit(c)} onDelete={()=>handleDelete(c.id,c.name)}/>)}</div>}
          ...
          <form onSubmit={handleSubmit} className="space-y-3 mt-2">
            <div className="space-y-1.5">
              ...
              <div className="flex items-center border rounded-md overflow-hidden">
                <span className="px-3 py-2 text-sm bg-muted border-r text-muted-foreground font-mono">{prefix}</span>
                <Input value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value.toLowerCase().replace(/[^a-z0-9-]/g,'')}))} placeholder="my-command" className="border-0 rounded-none" required/>
              </div>
              <p className="text-xs text-muted-foreground">Members use: <span className="font-mono">{prefix}{form.name || 'command-name'}</span></p>
            </div>
